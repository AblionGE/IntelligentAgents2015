...
The most expensive step of the algorithm in terms of running time is the function ```chooseNeighbours()``` which is $O(N_T + \left( \frac{N_T}{N_V}\right) ^3)$ if the tasks are well spread over the vehicles but at worst $O(N_V + N_T + N_T^3)$. The dependence of the number of tasks for the algorithm's complexity is then much more significative than the number of vehicles in the worst case and the increase of the number of vehicles is more likely to reduce the running time. It is illustrated in Figure \ref{fig:complexity}.
